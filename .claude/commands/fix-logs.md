...
Audit and fix all logging in a service so it complies with `.claude/docs/logging-strategy.md`.

> Requires the observability module (`.claude/docs/logging-strategy.md`). If it doesn't exist yet, run
...
1. **Audit** — apply the **`logging-compliance` skill** to the service's source. It checks required
   ...
   `.claude/docs/logging-strategy.md`.
