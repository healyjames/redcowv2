...
> This repo is a single Astro app (not a monorepo) — the one target is the app itself
> (`redcowv2`), covering `src/pages`, `src/components`, `src/libs`, and the per-brand
> folders under `src/assets/<brand>`. Targets and their layout come from
> `.claude/docs/workflow-config.md` (`## Audit`). `/install` (audit module) configures
> the target list, the output location, and prunes any dimensions that don't apply to
> this stack — here that means the `logging` dimension is dropped (no observability
> module installed).
...
Resolve the target list from `.claude/docs/workflow-config.md`. `all` → every configured target. A single name → just that target; if it doesn't match, list the valid targets and stop. Record the list. Frontend-only dimensions (e.g. `accessibility`) apply only to UI targets — skip them elsewhere.
...
| `error-handling` | Swallowed errors, missing try/catch at boundaries (API routes under `src/pages/api`, email sending in `src/libs/email`), retry/backoff, errors caught but silently discarded | `.claude/docs/coding.md` |
| `security` | OWASP Top 10, hardcoded secrets, missing input validation at trust boundaries (API routes, form submissions), injection, authz gaps | the `security-review` skill |
| `performance` | Unbatched/blocking calls, redundant fetches, missing caching, large payloads (images/fonts per brand), sync work in loops, unnecessary React hydration | the `performance-review` skill |
| `test-quality` | Coverage gaps on business logic **and** test quality: black-box vs implementation-coupled, misleading names, factory usage | `.claude/docs/testing.md` |
| `schema-consistency` | Validation at trust boundaries, producer/consumer schema drift, enum/shape values not shared where they should be | `.claude/docs/coding.md` |
| `dry-duplication` | Constants/lists/mapping logic duplicated across modules or brand folders that belong in a shared place (`src/libs/utils`, `src/libs/types`) | `.claude/docs/coding.md` |
| `documentation-drift` | Docs out of sync with the code | the target's own docs |
| `accessibility` | Public-facing multi-tenant restaurant site — WCAG/ARIA/keyboard/contrast/semantics across brand folders | `ai-accessibility` plugin if installed, else general WCAG review |
| `dependency-health` | **Repo-level, once per run** — dependency audit (`npm audit`), outdated majors, known CVEs | — |
