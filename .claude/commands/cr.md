...
1. **Ticket** — No ticket-tracker is configured for this project. If the branch name starts with
   a ticket-ID-shaped prefix (e.g. `PROJ-1234-...`), there's nothing to fetch it from — treat it
   as plain text context only (it may hint at the topic) and move on.
