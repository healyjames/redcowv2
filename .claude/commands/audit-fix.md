...
This repo is a single-app target (`redcowv2`, not a monorepo), so `<target>` is almost always
`redcowv2` — the whole repo.

## Arguments

$ARGUMENTS — `<target> <dimension>` and an optional `<date>`.

- `<target>` — the audited target. For this repo, that's `redcowv2` (the app itself).
...
4. No test/lint/format script is configured yet (see `.claude/docs/workflow-config.md`) — run `npm run build` and rely on manual verification (and `/run` if a visual check is needed); fix any failures. If Vitest has since been added as a devDependency (see `.claude/docs/testing.md`), also run it.
