...
Follow the project's testing guidelines in `.claude/docs/testing.md` (method: **TDD**, framework:
**Vitest** + **React Testing Library**, jsdom for React islands / node for API routes and
`src/libs`). See the `test-writer` agent for the framework mechanics and Astro-specific test
placement notes. The defaults below apply where `.claude/docs/testing.md` doesn't say more.

**Note:** `vitest`, `@testing-library/react`, `@testing-library/dom`, `@testing-library/jest-dom`,
and `jsdom` are not yet installed as devDependencies (see `.claude/docs/testing.md`) — write tests
to this spec regardless, and flag the missing tooling rather than silently skipping tests.

## TDD

This project follows test-driven development: write the failing test for the behaviour first, then
implement until it passes, then refactor with the test green. When a subtask's scope is a new
behaviour (not a pure refactor), the test-writing step in `/dev` should happen before or alongside
the implementation, not purely after.
...
- **Test business logic and boundaries; don't chase 100% line coverage of mid-function
  implementation** — that's usually wasted effort (unless `.claude/docs/testing.md` says otherwise).
