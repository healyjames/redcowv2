---
name: architecture
description: Keep new and changed code within this project's chosen architecture (hexagonal / layered / vertical slice / …) and its boundary rules. Reads .claude/docs/architecture.md. Use when writing or reviewing code that touches module boundaries, dependencies, or where logic lives.
allowed-tools: Read, Grep, Glob
---

# Architecture

Keep code within the project's architecture as recorded in `.claude/docs/architecture.md`. **Only applies if
that file exists** (scaffolded by `/install` or `/clarify architecture`); if it doesn't, skip.

## What to enforce

- **The pattern's dependency rule** — whatever `.claude/docs/architecture.md` names: hexagonal (the domain
